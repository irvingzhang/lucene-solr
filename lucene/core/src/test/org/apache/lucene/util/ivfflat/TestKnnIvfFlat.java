...
public class TestKnnIvfFlat extends LuceneTestCase {
  ...
  public void testBasic() throws Exception {
    try (Directory dir = newDirectory(); IndexWriter iw = new IndexWriter(
      ...
      for (int i = 0; i < numDoc; i++) {
        ...
        KnnTestHelper.add(iw, i, values[i], VectorValues.VectorIndexType.IVFFLAT);
      }

      KnnTestHelper.assertConsistent(iw, Arrays.asList(values), VectorValues.VectorIndexType.IVFFLAT);
    }
  }
  ...
  public void testSingleDocumentRecall() throws Exception {
    try (Directory dir = newDirectory(); IndexWriter iw = new IndexWriter(dir,
      ...
      KnnTestHelper.add(iw, 0, values[0], VectorValues.VectorIndexType.IVFFLAT);
      KnnTestHelper.assertConsistent(iw, Arrays.asList(values), VectorValues.VectorIndexType.IVFFLAT);
      iw.commit();
      KnnTestHelper.assertConsistent(iw, Arrays.asList(values), VectorValues.VectorIndexType.IVFFLAT);

      try (IndexReader reader = DirectoryReader.open(dir)) {
        KnnTestHelper.assertRecall(reader, 1, values[0],
            50, new int[]{0}, Arrays.asList(values));
      }
    }
  }
    ...
    try (IndexReader reader = DirectoryReader.open(dir)) {
      for (int i = 0; i < querySize; ++i) {
        result = KnnTestHelper.assertRecall(reader, topK, queryDataset.get(i),
            centroid, groundTruthVects.get(i), totalVecs);
  ...
  public static final class KnnTestHelper {
    ...
    public static void assertConsistent(IndexWriter iw, List<float[]> values, VectorValues.VectorIndexType type) throws IOException {
      try (DirectoryReader dr = DirectoryReader.open(iw)) {
        ...
        for (LeafReaderContext ctx : dr.leaves()) {
          ...
          if (type == VectorValues.VectorIndexType.IVFFLAT) {
    ...
    public static QueryResult assertRecall(IndexReader reader, int topK, float[] value,
                                           int centroids, int[] truth, final List<float[]> totalVecs) throws IOException {
      IndexSearcher searcher = new IndexSearcher(reader, null);
      Query query = new KnnIvfFlatQuery(KNN_VECTOR_FIELD, value, topK, centroids);
