...
public class Lucene90IvfFlatIndexWriter extends IvfFlatIndexWriter {
  protected final IndexOutput ivfFlatMeta, vectorData;
  ...
  public Lucene90IvfFlatIndexWriter(SegmentWriteState state) throws IOException {
    ...
    final String vecDataFileName = IndexFileNames.segmentFileName(state.segmentInfo.name,
        state.segmentSuffix, Lucene90IvfFlatIndexFormat.VECTOR_DATA_EXTENSION);
    ...
    try {
      ...
      CodecUtil.writeIndexHeader(vectorData, Lucene90IvfFlatIndexFormat.VECTOR_DATA_CODEC_NAME,
          Lucene90IvfFlatIndexFormat.VERSION_CURRENT, state.segmentInfo.getId(),
          state.segmentSuffix);
  ...
  protected void mergeOneField(FieldInfo fieldInfo, MergeState state) throws IOException {
    ...
    final List<VectorValuesSub> subs = new ArrayList<>(readerLength);
    for (int i = 0; i < readerLength; ++i) {
      ...
      if (ivfFlatIndexReader != null) {
        subs.add(new VectorValuesSub(i, state.docMaps[i], ivfFlatIndexReader.getVectorValues(fieldInfo.name)));
      }
    }
